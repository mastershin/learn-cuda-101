...
def parse_args():
    ...
    parser.add_argument('size', type=str, nargs='?', help='Matrix size: s (small), m (medium), l (large) or specify m, n, k', default='s')
    parser.add_argument('dimensions', type=int, nargs='*', help='Specify dimensions m, n, k')
...
if __name__ == "__main__":
    ...
    if args.size in ['s', 'm', 'l'] and not args.dimensions:
        if args.size == 's':
            ...
            raise ValueError("Invalid size argument. Use 's', 'm', 'l' or specify dimensions.")
    elif len(args.dimensions) == 3:
        m, n, k = args.dimensions
    else:
        raise ValueError("Invalid arguments. Use 's', 'm', 'l' for predefined sizes or specify dimensions m, n, k.")
