...
__host__ void copyFromHostToDevice(std::string kernelType, int *input, int numElements, int numThreads, int *d_input)
{
    ...
    if(!strcmp(kernelType.c_str(), "constant"))
    {
        ...
        cudaMemcpyToSymbol((const char*)&constant_num_elements, &numElements, sizeof(int));
        cudaMemcpyToSymbol((const char*)&constant_num_threads, &numThreads, sizeof(int));

        int threadSpan = numElements / numThreads;
        cudaMemcpyToSymbol((const char*)&constant_thread_span, &threadSpan, sizeof(int));
    } else 
    {
